...
#set alarm levels
max_time_door_forward = 40
max_time_door_backward = 18
...
def get_date_times():
    ...
    today_sr = today_sr + timedelta(minutes=60)
    today_ss = today_ss + timedelta(minutes=60)
    time_open_door = today_sr + timedelta(minutes=15)
    ...
    return date_now, time_now, time_open_door, time_close_door, today_sr, today_ss

@anvil.server.callable
def get_coop_temp_hum():
    coop_temp = round(GHT001.temperature,2)
    coop_hum = round(GHT001.relative_humidity,2)
...
def open_coop_door():
    global door_status
    door_motor.forward()
    for i in range(0,max_time_door_forward,1):
        i += 1
        time.sleep(1)
        print(i)
        if GPIO.input(LS001):
...
def close_coop_door():
    global door_status
    door_motor.backward()
    for i in range(0,max_time_door_backward,1):
        i += 1
        print(i)
        time.sleep(1)
        if GPIO.input(LS002):
